...
namespace ordertest
{
    ...
    public class OrderService
    {
        ...
        public void Import()//读入
        {
            ...
            orderList.Clear();
            XElement XDoc = XElement.Load(@"order.xml");


            var elements = from element in XDoc.Elements("Order") select element;
            foreach (var e in elements)
            {
                Order order = new Order();
                order.Id = uint.Parse(e.Element("Id").Value);
                    elements = from element in e.Elements("Customer") select element;
                    var f = elements.ElementAt(0);
                    Customer customer = new Customer();
                    customer.Id = uint.Parse(f.Element("Id").Value);
                    customer.Name = f.Element("Name").Value;
                order.Customer = customer;
                    elements = from element in e.Elements("Details") select element;
                    f = elements.ElementAt(0);
                        elements = from element in f.Elements("OrderDetail") select element;
                        foreach (var g in elements)
                        {
                            OrderDetail detail = new OrderDetail();
                            detail.Id = uint.Parse(g.Element("Id").Value);
                            detail.Quantity = uint.Parse(g.Element("Quantity").Value);
                                elements = from element in g.Elements("Goods") select element;
                                f = elements.ElementAt(0);
                                Goods goods = new Goods();
                                goods.Id = uint.Parse(f.Element("Id").Value);
                                goods.Price = double.Parse(f.Element("Price").Value);
                                goods.Name = f.Element("Name").Value;
                            detail.Goods = goods;
